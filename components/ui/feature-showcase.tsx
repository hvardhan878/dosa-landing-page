...
export function FeatureShowcase({
  ...
  return (
    <section className={cn("w-full bg-background text-foreground", className)}>
      <div className="container mx-auto grid max-w-7xl grid-cols-1 gap-10 px-6 py-16 md:grid-cols-12 md:py-24 lg:gap-14">
        {/* Left column */}
        <div className="md:col-span-6">
          ...
          {description ? (
            <p className="mt-6 max-w-xl text-gray-400">{description}</p>
          ) : null}
            ...
            <Accordion type="single" collapsible className="w-full">
              {steps.map((step) => (
                <AccordionItem key={step.id} value={step.id}>
                  <AccordionTrigger className="text-left text-base font-medium text-white hover:text-purple-300 transition-colors">
                    {step.title}
                  </AccordionTrigger>
        ...
        <div className="md:col-span-6">
          <Card
            className="relative overflow-hidden rounded-2xl border border-white/10 bg-black/40 p-0 shadow-sm backdrop-blur-sm"
          ...
          >
            <Tabs defaultValue={initial} className="relative h-full w-full">
              ...
              <div className="pointer-events-auto absolute inset-x-0 bottom-4 z-10 flex w-full justify-center">
                <TabsList className="flex gap-2 rounded-xl border border-white/20 bg-black/80 p-1 backdrop-blur supports-[backdrop-filter]:bg-black/70">
                  {tabs.map((t) => (
                    <TabsTrigger
                      key={t.value}
                      value={t.value}
                      className="rounded-lg px-4 py-2 text-white/80 data-[state=active]:bg-white data-[state=active]:text-black transition-colors"
                    >
                      {t.label}
                    </TabsTrigger>
                  ))}
